...
class BillApp extends FrontendApp {
		...
		if (!IS_POST) {
			...
			if (!is_array($_POST['account_id'])) {
				...
				if ($_POST['inter_mediate'] > 0) {
					...
					$account_list[] = array(
						'account_id'	=> $_POST['inter_mediate'],
						'type'			=> Event_accountModel::TYPE_COMPLEX,
						'amount'		=> 0 - $amount,
						'comment'		=> $comment,
					);
					$pay_id = $_POST['inter_mediate'];
				} else {
					$account_list[] = array(
						'account_id'	=> $account_id,
						'type'			=> Event_accountModel::TYPE_COMPLEX,
			...
			$all_account = $this->_amod->getList($this->_user_id);
			$account_ids = array_column($account_list, 'account_id');
			foreach ($account_ids  as $account_id) {
				if (!$all_account[$account_id]) {
					$this->show_warning('错误的账号');
				}
			}//}}}
			//{{{处理分账
			$outer_account = array();
			foreach ($all_account as $one) {
				if ($one['outer_user_id'] && !array_key_exists($one['outer_user_id'], $outer_account)) {
					$outer_account[$one['outer_user_id']] = $one['account_id'];
				}
			}
			if (is_array($_POST['consumer_amount']) && count($_POST['consumer_amount'])) {
				foreach ($_POST['consumer'] as $index => $outer_user_id) {
					if ('self' == $outer_user_id) {
						$expense_data = array(
							'user_id'		=> $this->_user_id,
							'expense_name'	=> $comment,
							'type'			=> 99,//todo
							'amount'		=> $_POST['consumer_amount'][$index],
							'buy_date'		=> $date,
							'comment'		=> $_POST['consumer_comment'][$index],
						);
						$tmp_amount += $_POST['consumer_amount'][$index];
					} else {
						$account_list[] = array(
							'account_id'	=> $outer_account[$outer_user_id],
							'type'			=> Event_accountModel::TYPE_TRANSFER_IN,
							'amount'		=> $_POST['consumer_amount'][$index],
							'comment'		=> $_POST['consumer_comment'][$index],
							'extra'			=> $pay_id,
				...
				if (in_array('self', $_POST['consumer'])) {
					$expense_data = array(
						...
						'expense_name'	=> $comment,
						'type'			=> 99,//todo
						'amount'		=> $amount - $average * ($user_count - 1),
						'buy_date'		=> $date,
						'comment'		=> $comment,
					);
					$_POST['consumer'] = array_diff($_POST['consumer'], array('self'));
					foreach ($_POST['consumer'] as $outer_user_id){
						$account_list[] = array(
							'account_id'	=> $outer_account[$outer_user_id],
							'type'			=> Event_accountModel::TYPE_TRANSFER_IN,
							'amount'		=> $average,
					...
					$last_amount = $amount - $average * ($user_count -1);
					for ($i=0; $i<$user_count; $i++) {
						if ($i == ($user_count-1)) {
							$account_list[] = array(
								'account_id'	=> $outer_account[$_POST['consumer'][$i]],
								'type'			=> Event_accountModel::TYPE_TRANSFER_IN,
								'amount'		=> $last_amount,
								'comment'		=> $comment,
								'extra'			=> $pay_id,
			...
			$event_data = array(
				'user_id'		=> $this->_user_id,
				'position_id'	=> $position_id,
				'type'			=> $expense_data ? EventModel::TYPE_EXPENSES : EventModel::TYPE_DINNER,
			...
			$event_id = $this->_emod->add($event_data);
			if (!$event_id) {
				$this->show_warning('聚餐事件记录失败');
			}
			//记录支出详情和处理账户余额
			if ($expense_data) {
				$expense_data['event_id'] = $event_id;
				if (!$this->_exmod->add($expense_data)) {
					$this->_db_rollback();
					$this->show_warning('消费事件记录失败');
				}
			}
			foreach ($account_list as $account) {
				...
				if ($account['amount'] > 0) {
					$account_data = array(
						'income'	=> $all_account[$account['account_id']]['income'] + $account['amount'],
						'balance'	=> $all_account[$account['account_id']]['balance'] + $account['amount'],
					);
				} else {
					$account_data = array(
						'expense'	=> $all_account[$account['account_id']]['expense'] - $account['amount'],
						'balance'	=> $all_account[$account['account_id']]['balance'] + $account['amount'],
					);
				}
				if (!$this->_amod->edit($account['account_id'], $account_data)) {
	...
	function ajax_getconsumer() {
        $q = trim($_REQUEST['q']);
		$conditions = array(
			'user_id='.$this->_user_id,
			"user_name like '%$q%'",
		);
		$res = $this->_oumod->get_options(implode(' AND ', $conditions), 'user_name', 'outer_user_id');
		$data = array();
