...
/*
除本基类文件外，所有的模型类的类名的构造规则应该是模型名(首字母大写)+mmodel组成，文件名必须是模型名+.mmodel组成
如有一个用户模型，模型名为user，则其文件名应为user.mmodel.php，类名为UserMModel
*/
class BaseMModel extends Object
{
    var $db = null;
    ...
    function __construct($params, $mdb)
    {
        $this->BaseMModel($params, $mdb);
    }
    /**
     *  构造函数
     *
     ...
     *  @return void
     */
    function BaseMModel($params, $mdb)
    {
        $this->db =& $mdb;
        !$this->alias && $this->alias = $this->table;
        $this->_prefix = MDB_PREFIX;
