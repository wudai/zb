...
class BaseModel extends Object
{
    ...
    function find($params = array())
    {
        ...
        if ($this->prikey && ($index_key == $this->prikey || (is_array($index_key) && in_array($this->prikey, $index_key))))
        {
		...
		if (is_array($params) && array_key_exists('debug', $params) && $params['debug']) {
			echo $sql, "<br />\n";
		}
     ...
     *  @return int
     */
    function getCount($params = array(), $col = '')
    {
		if (!$params) {
     ...
     * @return array
     */
    function getRelatedDataCount($relation_name, $ids, $col = '', $find_param = array())
    {
        $relation_info = $this->getRelation($relation_name);
        $model =& m($relation_info['model']);
        if (empty($ids))
        ...
        $conditions .= $relation_info['ext_limit'] ?
                            ...
                            : '';
        $conditions .= is_string($find_param['conditions']) ? ' AND ' . $find_param['conditions'] : '';
		$conditions .= ' GROUP BY '.$alias . '.' . $relation_info['foreign_key'];
        $find_param['conditions'] = $conditions;


        /* 查询字段 */
		if ($col) {
			$find_param['fields'] = 'COUNT('.$col.') AS count,' . $alias . '.' .$relation_info['foreign_key'];
		} else {
			$find_param['fields'] = 'COUNT(*) AS count,' . $alias . '.' .$relation_info['foreign_key'];
		}
        switch ($relation_info['type'])
        {
            case HAS_MANY:
		...
		$find_param['index_key'] = $relation_info['foreign_key'];
